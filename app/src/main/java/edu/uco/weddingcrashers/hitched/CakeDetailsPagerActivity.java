 ...
 */
public class CakeDetailsPagerActivity extends AppCompatActivity {
    ...
    public static Intent newIntent(Context packageContext,UUID vendorID){
        Intent intent = new Intent(packageContext,CakeDetailsPagerActivity.class);
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_vendor_details_pager_cake);
        ...
        mCakeVs = CakeList.get(this).getVendors();
        FragmentManager fragmentManager = getSupportFragmentManager();
        mViewPager.setAdapter(new FragmentStatePagerAdapter(fragmentManager) {
            @Override
            public Fragment getItem(int position) {
                CakeV cakeV = mCakeVs.get(position);
                return CakeDetailFragment.newInstance(cakeV.getVendorID());
        ...
        for(int i = 0;i< mCakeVs.size();i++){
            if(mCakeVs.get(i).getVendorID().equals(vendorID)){
