 ...
 */
public class PhotographerFragment extends Fragment implements GoogleApiClient.OnConnectionFailedListener {
...
/*    private Place mPlace;*/
    ...
    List<PhotographerReview> mPhotographerReviewList;

    public static PhotographerFragment newInstance(String placeID, String iconURL) {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()){
            case R.id.menu_item_save_vendor:
                ...
                photographerSaved.setId(placeID);
                photographerSaved.setName(mName.getText().toString());
                photographerSaved.setAddress(mAddress.getText().toString());
                photographerSaved.setPhone(mPhone.getText().toString());
                photographerSaved.setRating(String.valueOf(mRatingBar.getRating()));
                photographerSaved.setWebsite(mWeb.getText().toString());
                if(iconURL == null)
                    iconURL="Unknown";
                photographerSaved.setImgURL(iconURL);
                boolean flag = PhotographerSavedList.get(getActivity()).addSavedVendor(photographerSaved);
                if(flag == true)
            ...
            case R.id.menu_item_favorite:
                Intent i = new Intent(getActivity(),PhotographerPSavedListActivity.class);
                startActivity(i);
                return true;
            ...
            case R.id.menu_item_review:
                Intent reviewIntent = new Intent(Intent.ACTION_VIEW, Uri.parse(mPhotographerReviewList.get(0).getUrl()));
...
//                PhotographerSaveFragment dialog = PhotographerSaveFragment.newInstance(mPhotographerReviewList.get(0).getUrl());
//                dialog.show(manager,"DIALOG_RATE");
    ...
    private class FetchReviewTask extends AsyncTask<Void,Void,List<PhotographerReview>> {

        @Override
        protected List<PhotographerReview> doInBackground(Void... voids) {
            return new PhotographerPlaceFetchr().fetchVendorReview(placeID);

        }

        @Override
        protected void onPostExecute(List<PhotographerReview> photographerReviewList) {
            mPhotographerReviewList = photographerReviewList;
            for(int i = 0;i< photographerReviewList.size();i++){
                mReview.append("\n");
                mReview.append((i+1)+". Name: " + photographerReviewList.get(i).getAuthorName());
                mReview.append("\n");
                mReview.append("Rating: "+ photographerReviewList.get(i).getRating());
                mReview.append("\n");
                mReview.append("Time: "+ photographerReviewList.get(i).getTime());
                mReview.append("\n");
                mReview.append(photographerReviewList.get(i).getText());
                mReview.append("\n");
            }
        }
    }
