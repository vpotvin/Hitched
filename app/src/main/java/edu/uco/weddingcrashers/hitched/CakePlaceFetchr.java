 ...
 */
public class CakePlaceFetchr {

    private static final String TAG = "CakePlaceFetchr";
    ...
    public List<CakeReview> fetchVendorReview(String placeid){
        List<CakeReview> cakeReviewList = new ArrayList<>();
        try{
            ...
            parseReview(cakeReviewList,jsonBody);
        }catch (JSONException je){
            Log.e("VendorReview","Failed to parseJSON",je);
        }catch (IOException ioe){
            Log.e("VendorReview","Failed to fetch rating",ioe);
        }
        return cakeReviewList;
    }

    public List<CakePlace> fetchItems(String querySearch) {
        List<CakePlace> items = new ArrayList<>();
    ...
    private void parseReview(List<CakeReview> items,JSONObject jsonBody) throws IOException,JSONException{
        ...
        for(int i = 0;i<reviewJsonArray.length();i++){
            CakeReview mCakeReview = new CakeReview();
            JSONObject reviewJsonObject = reviewJsonArray.getJSONObject(i);
            Log.i("TAG","review found:" + reviewJsonObject.getString("text"));
            mCakeReview.setAuthorName(reviewJsonObject.getString("author_name"));
            mCakeReview.setRating(reviewJsonObject.getString("rating"));
            mCakeReview.setText(reviewJsonObject.getString("text"));
            mCakeReview.setTime(reviewJsonObject.getString("time"));
            mCakeReview.setUrl(resultJsonObject.getString("url"));
            items.add(mCakeReview);
...
//            reviews.add(reviewJasonObject.getString("text"));
    ...
    private void parseItems(List<CakePlace> items, JSONObject jsonBody) throws IOException, JSONException {
        JSONArray resultJsonArray = jsonBody.getJSONArray("results");

        for (int i = 0; i < resultJsonArray.length(); i++) {
            CakePlace item = new CakePlace();
